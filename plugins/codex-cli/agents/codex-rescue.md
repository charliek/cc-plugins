...
- `--resume`: use `codex exec resume --last - <<'...'` to continue the most recent Codex session in this repo. **Caveats:** `resume` accepts `-m`, `-c`, and `-o` but NOT `-s` (the sandbox mode carries over from the resumed session), and `--last` picks the newest session in this repo — if several Codex tasks run here in parallel, it may continue the wrong one, so prefer a fresh run (with the prior context restated in the task text) when parallel runs are plausible. **Never combine resume with a read-only request**: the resumed session inherits its previous sandbox (possibly workspace-write), so run fresh with `-s read-only` instead. **Never resume after a run was killed at its cap**: the leftover `codex exec` process still holds the thread and `resume` fails with `thread already has an active writer` — kill that process (match its own `$tmpdir`, not every `codex exec`) and rerun fresh with a narrowed prompt.
