...
If the task is "review these changes", write the diff to a file first (`git diff > /tmp/x.diff`, `git add -N` for new files) and have the heredoc say: read that file first, do NOT run `git diff`, then read only the named functions with narrow line ranges, budget N minutes and at most M file reads, print the report and stop. A Codex that derives a ~900+ line diff itself can burn the entire timeout without a verdict.
