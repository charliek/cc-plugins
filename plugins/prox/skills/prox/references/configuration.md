...
Custom domains require DNS entries pointing to `127.0.0.1`. Options include:
- `lvh.me` — free DNS service that resolves all subdomains to 127.0.0.1 (no setup required)
- `/etc/hosts` — manual entries
- `prox hosts --add` — prox can manage /etc/hosts entries automatically (requires sudo)
