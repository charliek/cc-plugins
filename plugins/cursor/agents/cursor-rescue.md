...
Default model: `cursor-grok-4.5-high`. Use it unless the user explicitly asks for a different model. Discover ids with `agent --list-models`.
...
- Default to a write-capable run. **Always use a per-invocation random delimiter** — append fresh random hex to the base token (shown here as `CURSOR_TASK_9f3a2b1c`) and never use the bare `CURSOR_TASK`. A unique suffix the caller cannot predict makes it impossible for the task text to terminate the heredoc early:
  ...
  ```bash
  agent -p --force --model cursor-grok-4.5-high <<'CURSOR_TASK_9f3a2b1c'
...
- `--model <id>`: pass it through to `agent --model <id>`, replacing the `cursor-grok-4.5-high` default. There is no `--effort` flag and no `spark` alias for Cursor — reasoning level is encoded in the model id (e.g. `cursor-grok-4.5-high`, `gpt-5.6-sol-high`).
