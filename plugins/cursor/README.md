...
All commands default to the **`cursor-grok-4.6-high-fast`** model, stated near the top of each command/subagent file (when bumping the default, update every command file plus this README). Override per call with `--model <id>`. Cursor encodes reasoning level in the model id (e.g. `cursor-grok-4.6-high-fast`, `gpt-5.6-sol-high`) — there is no separate effort flag. Run `agent --list-models` to discover available ids.
